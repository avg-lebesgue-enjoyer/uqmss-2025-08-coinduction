...
\begin{frame}{All the categories} % .;
  ...
  \begin{block}{Initial algebras}
    ...
    \pause
    \par An \strong{algebra} for $T$ is an arrow $T(a) \xrightarrow{\alpha} a$ in $\mathcal{C}$.
    \vspace{0.5 \baselineskip}
    \pause
    \par A \strong{morphism} from $T(a) \xrightarrow{\alpha} a$ to $T(b) \xrightarrow{\beta} b$ is an arrow $f : a \to b$ in $\mathcal{C}$ such that this commutes:
    \begin{align*}
...
\begin{frame}{Recursion/Induction} % .;
  ...
  \begin{block}{Recursion}
    ...
    \par An \strong{initial algebra} for $T$ is an initial object $T(i) \xrightarrow{\cons} i$ in the category of $T$-algebras.
    \vspace{0.5 \baselineskip}
    \pause
